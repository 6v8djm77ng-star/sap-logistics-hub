...
function applyCleanup(s) {
  ...
  // Also drop any DNs/Invoices that the seed produced when the e2e ran.
  // Filter by RunNumber prefix (most reliable) AND by IsTest tag.
  const before_dn = (s.deliveryNotes || []).length;
  s.deliveryNotes = (s.deliveryNotes || []).filter((d) => {
    if (!d) return false;
    if (d.IsTest === true) return false;
    if (typeof d.RunNumber === 'string' && d.RunNumber.startsWith(TEST_RUN_PREFIX)) return false;
    if (typeof d.CardCode === 'string' && d.CardCode === TEST_CARDCODE) return false;
    return true;
  });
  removed.deliveryNotes = before_dn - s.deliveryNotes.length;
  const before_inv = (s.invoices || []).length;
  s.invoices = (s.invoices || []).filter((i) => {
    if (!i) return false;
    if (i.IsTest === true) return false;
    if (typeof i.CardCode === 'string' && i.CardCode === TEST_CARDCODE) return false;
    return true;
  });
