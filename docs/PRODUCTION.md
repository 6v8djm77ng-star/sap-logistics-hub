...
- משתמש: `admin` / סיסמה: סיסמה חזקה נקבעת בעת ההתקנה ונשמרת אצל מנהל המערכת בלבד (Bitwarden / כספת ארגונית). אסור לתעד סיסמאות במסמכים, ב-README, או ב-git.
- **שנה סיסמה מיד אם זוהי התקנה ראשונה** (משתמשים → admin → איפוס סיסמה)
