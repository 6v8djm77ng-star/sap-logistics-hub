...
import { Package, Search, Building2, Calendar, X, ChevronDown, ChevronUp, Eye, FileText } from 'lucide-react';
    ...
    <>
      <tr className="hover:bg-gray-50 cursor-pointer" onClick={() => setExpanded(!expanded)}>
        ...
        <td className="px-3 py-2 text-sm">
          ...
          <div className="text-xs text-gray-500 font-mono">{order.CardCode}</div>
        </td>
...
export default function OpenOrdersPage() {
  ...
  // Apply client-side sort (city / zone require parsing) so the user can flip
  // sort order without re-querying the server.
  const orders = (() => {
    const arr = [...rawOrders];
    const cityOf = (o) => {
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-100">
                {orders.map((order) => (
                  <OrderDetailsRow key={`${order.CompanyCode}-${order.DocEntry}`} order={order} />
                ))}
              </tbody>
            </table>
          </div>
